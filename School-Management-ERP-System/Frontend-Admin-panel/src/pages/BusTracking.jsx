...
import {
  Bus, Phone, MapPin, User, Users, Navigation, Search, X, Save, Pencil,
  Camera, Video, Gauge, Headset, Radio
} from "lucide-react";
import { PageIntro, Card, Button, Input, Select, Pill, statusTone, StatCard } from "../components/UI";
import { busRoutes as initialRoutes, transportDesk } from "../data/academics";
        ...
        routes.forEach((r) => {
          ...
          const wp =
            r.waypoints && r.waypoints.length
              ? r.waypoints
              : [[r.lat, r.lng], [SCHOOL_LOCATION.lat, SCHOOL_LOCATION.lng]];
          const cycle = 20000;
          ...
          const s = Math.max(0, (r.status === "Delayed" ? 12 : 18) + Math.random() * 20);
...
function FleetMap({ routes, positions, selected, onSelect }) {
  ...
  useEffect(() => {
    ...
    const map = L.map(containerRef.current, { center: [SCHOOL_LOCATION.lat, SCHOOL_LOCATION.lng], zoom: 12 });
    L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
      maxZoom: 19, attribution: "&copy; OpenStreetMap contributors",
    }).addTo(map);
    L.marker([SCHOOL_LOCATION.lat, SCHOOL_LOCATION.lng], { icon: makeSchoolIcon(), zIndexOffset: 1000 })
      .addTo(map)
      .bindTooltip("Brightwood International School", { direction: "top", offset: [0, -20] });
    mapRef.current = map;
    return () => {
      map.remove();
      mapRef.current = null;
  ...
  useEffect(() => {
    ...
    if (latlngs.length) mapRef.current.fitBounds(latlngs, { padding: [50, 50] });
  }, [routes]);

  useEffect(() => {
    if (!mapRef.current) return;
    routes.forEach((r) => {
      const latlngs = (r.waypoints && r.waypoints.length ? r.waypoints : [[r.lat, r.lng]]).map((p) => [p[0], p[1]]);
      const color = STATUS_COLOR[r.status] || "#94A3B8";
      if (pathRefs.current[r.id]) {
        pathRefs.current[r.id].setLatLngs(latlngs);
        pathRefs.current[r.id].setStyle({ color, dashArray: r.status === "Delayed" ? "6 6" : null });
      } else {
        pathRefs.current[r.id] = L.polyline(latlngs, { color, weight: 3, opacity: 0.6, dashArray: r.status === "Delayed" ? "6 6" : null }).addTo(mapRef.current);
      }
    });
  }, [routes]);
  ...
  useEffect(() => {
    ...
    routes.forEach((r) => {
      ...
      if (markersRef.current[r.id]) {
        ...
        const marker = L.marker([pos.lat, pos.lng], { icon: makeIcon(STATUS_COLOR[r.status] || "#94A3B8"), riseOnHover: true });
        marker.bindTooltip(`${r.id} · ${r.route.split(" — ")[0]}`, { direction: "top", offset: [0, -22] });
      ...
      if (el) {
        ...
        if (dot) dot.classList.toggle("is-selected", selected && r.id === selected.id);
      }
    });
  ...
  return (
    <div className="relative flex-1 min-h-[520px]">
      ...
      <div className="absolute top-3 left-3 z-[400] bg-white/95 backdrop-blur rounded-xl shadow px-3 py-2 text-[11px] font-semibold text-ink">
        <span className="inline-flex items-center gap-1.5"><Radio size={12} className="text-success" /> LIVE</span> · Bhopal Fleet · GPS refresh 150ms
      </div>
...
function ContactRow({ icon: Icon, label, name, phone, highlighted }) {
  return (
    <div className="flex items-center gap-3">
      <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${highlighted ? "bg-amber/15 text-amber-dark" : "bg-paper text-slate-text/70"}`}>
        <Icon size={16} />
      </div>
...
function CctvGrid({ bus, onOpen }) {
  return (
    <div className="grid grid-cols-2 gap-2">
      {Array.from({ length: bus.cctvCount || 4 }).slice(0, 6).map((_, i) => {
        ...
        return (
          ...
          >
            ...
            <div className="cctv-scene absolute bottom-0 left-0 right-0 h-1/3">
              <span className="cctv-car" style={{ animationDelay: `${i * 0.4}s` }} />
              <span className="cctv-car cctv-car-2" style={{ animationDelay: `${i * 0.7}s` }} />
            </div>
            <div className="absolute left-1.5 top-1.5 flex items-center gap-1">
              <span className="cctv-rec text-[9px] font-bold text-white/90 flex items-center gap-1"><span className="rec-dot" /> LIVE</span>
            </div>
...
export default function BusTracking() {
  ...
  const stats = useMemo(() => {
    const active = routes.filter((b) => b.status !== "Not Started" && b.status !== "Arrived").length;
  ...
  const speedOf = (r) =>
    positions[`${r.id}_speed`] !== undefined ? positions[`${r.id}_speed`] : r.speed;
  ...
  const handleSave = () => {
    setRoutes((prev) => prev.map((b) => (b.id === form.id ? { ...b, ...form } : b)));
    setSelected(form);
    setShowModal(false);
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={Bus} label="Active Buses" value={`${stats.active} / ${routes.length}`} accent="success" />
        <StatCard icon={Users} label="Students Onboard" value={String(stats.onboard)} accent="amber" />
        <StatCard icon={Navigation} label="On-Time Routes" value={String(stats.onTime)} accent="info" />
        <StatCard icon={MapPin} label="Delayed Routes" value={String(stats.delayed)} accent="alert" />
      </div>
        ...
        <div className="space-y-4">
          ...
          <Card title={`${activeBus.id} · Live`} bodyClassName="p-4">
            <div className="flex items-center justify-between mb-3">
              <Pill tone={statusTone(activeBus.status)}>{activeBus.status}</Pill>
              <div className="flex items-center gap-1">
                <span className="inline-flex items-center gap-1 text-[10.5px] font-bold text-success"><span className="rec-dot" /> LIVE</span>
                <button onClick={() => openEdit(activeBus)} className="p-1.5 rounded-lg hover:bg-paper text-slate-text/60 hover:text-info transition-colors" title="Edit bus">
                  <Pencil size={14} />
                </button>
              </div>
            </div>

            <p className="text-[13px] font-semibold text-ink leading-snug">{activeBus.route}</p>

            <div className="grid grid-cols-3 gap-2 mt-3">
              <Metric label="Occupancy" value={`${activeBus.occupied}/${activeBus.capacity}`} icon={Users} />
              <Metric label="ETA" value={activeBus.eta} icon={Navigation} />
              <Metric label="Speed" value={`${speedOf(activeBus)} km/h`} icon={Gauge} />
            </div>
            ...
            <div className="mt-3 rounded-lg bg-paper px-3 py-2 text-[11.5px] text-slate-text/70 flex items-center gap-2">
              ...
              <span className="font-mono">
                {positions[activeBus.id] ? positions[activeBus.id].lat.toFixed(5) : activeBus.lat},{positions[activeBus.id] ? positions[activeBus.id].lng.toFixed(5) : activeBus.lng}
              </span>
            </div>
          </Card>
          ...
          <Card title="Contacts" bodyClassName="p-4 space-y-3">
            <ContactRow icon={User} label="Driver" name={activeBus.driver} phone={activeBus.driverPhone} highlighted />
            <ContactRow icon={User} label="Conductor" name={activeBus.conductor} phone={activeBus.conductorPhone} />
            <ContactRow icon={Headset} label="Transport Desk" name={transportDesk.name} phone={transportDesk.phone} />
          </Card>
          ...
          <Card
            ...
            bodyClassName="p-4"
          >
            <CctvGrid bus={activeBus} onOpen={(b, i) => { setCctvBus(b); setCctvCam(i); }} />
          </Card>
        </div>
      </div>
        ...
        action={
          <div className="flex flex-wrap items-center gap-2">
            <div className="relative">
              ...
              <Input placeholder="Search route, driver..." value={query} onChange={(e) => setQuery(e.target.value)} className="pl-8 w-48" />
            </div>
      ...
      >
        {filtered.length === 0 ? (
          <div className="py-14 text-center">
            ...
            <p className="text-[13px] text-slate-text/60 mt-1">Try changing filters.</p>
          </div>
        ) : (
              ...
              <tbody>
                {filtered.map((b) => (
                  <tr key={b.id} onClick={() => setSelected(b)} className="border-b border-black/[0.04] hover:bg-paper/60 cursor-pointer">
                    ...
                    <td className="px-5 py-3 text-slate-text">{speedOf(b)} km/h</td>
                    <td className="px-5 py-3 text-slate-text">{b.eta}</td>
                    <td className="px-5 py-3">
                      <button
                        onClick={(e) => { e.stopPropagation(); setCctvBus(b); setCctvCam(0); }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-black/[0.06]">
              <div>
                <h3 className="font-display font-semibold text-ink text-[17px]">Edit {form.id}</h3>
                <p className="text-[12.5px] text-slate-text/70 mt-0.5">Update route, occupancy and status details.</p>
              </div>
              <button onClick={() => setShowModal(false)} className="p-2 rounded-lg hover:bg-paper text-slate-text"><X size={20} /></button>
            </div>
            <div className="px-5 py-4 space-y-4 max-h-[70vh] overflow-y-auto">
              <div>
                <label className="text-[12px] font-semibold text-ink mb-1.5 block">Route</label>
                <Input placeholder="e.g. Arera Colony — MP Nagar — School" value={form.route} onChange={(e) => updateForm("route", e.target.value)} />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[12px] font-semibold text-ink mb-1.5 block">Driver</label>
                  <Input value={form.driver} onChange={(e) => updateForm("driver", e.target.value)} />
                </div>
                <div>
                  <label className="text-[12px] font-semibold text-ink mb-1.5 block">Conductor</label>
                  <Input value={form.conductor} onChange={(e) => updateForm("conductor", e.target.value)} />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[12px] font-semibold text-ink mb-1.5 block">Driver Phone</label>
                  <Input value={form.driverPhone} onChange={(e) => updateForm("driverPhone", e.target.value)} />
                </div>
                <div>
                  <label className="text-[12px] font-semibold text-ink mb-1.5 block">Conductor Phone</label>
                  <Input value={form.conductorPhone} onChange={(e) => updateForm("conductorPhone", e.target.value)} />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[12px] font-semibold text-ink mb-1.5 block">Occupied</label>
                  <Input type="number" min={0} value={form.occupied} onChange={(e) => updateForm("occupied", Number(e.target.value))} />
                </div>
                <div>
                  <label className="text-[12px] font-semibold text-ink mb-1.5 block">Capacity</label>
                  <Input type="number" min={0} value={form.capacity} onChange={(e) => updateForm("capacity", Number(e.target.value))} />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[12px] font-semibold text-ink mb-1.5 block">ETAL</label>
                  <Input value={form.eta} onChange={(e) => updateForm("eta", e.target.value)} />
                </div>
                <div>
              ...
              <div>
                <label className="text-[12px] font-semibold text-ink mb-1.5 block">Last Stop</label>
                <Input value={form.lastStop} onChange={(e) => updateForm("lastStop", e.target.value)} />
              </div>
            </div>
            <div className="px-5 py-4 border-t border-black/[0.06] flex justify-end gap-2">
              <Button variant="outline" onClick={() => setShowModal(false)}>Cancel</Button>
              <Button variant="amber" onClick={handleSave}><Save size={15} /> Save Changes</Button>
            </div>
          </div>
        </div>
      )}
      ...
      {cctvBus && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-ink/85 backdrop-blur-sm" onClick={() => setCctvBus(null)} />
          <div className="relative bg-ink-dark rounded-2xl w-full max-w-3xl overflow-hidden border border-white/10">
            {/* header */}
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-white/10">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-lg bg-alert/15 text-alert flex items-center justify-center"><Video size={18} /></div>
                <div>
                  <p className="font-display font-semibold text-white text-[15px]">
                    {cctvBus.id} · CAM {String(cctvCam + 1).padStart(2, "0")} — {CAMERA_POSITIONS[cctvCam % CAMERA_POSITIONS.length].label}
                  </p>
                  <p className="text-[11.5px] text-white/50">{cctvBus.route}</p>
                </div>
              </div>
              <button onClick={() => setCctvBus(null)} className="p-2 rounded-lg hover:bg-white/10 text-white/70"><X size={20} /></button>
            </div>
            ...
            <div className="px-4 py-3 border-t border-white/10 flex flex-wrap items-center gap-2">
              {Array.from({ length: cctvBus.cctvCount || 4 }).slice(0, 6).map((_, i) => (
                <button
                  key={i}
                  onClick={() => setCctvCam(i)}
                  className={`px-3 py-1.5 rounded-lg text-[11.5px] font-semibold transition-colors ${
                    cctvCam === i ? "bg-amber text-ink" : "bg-white/5 text-white/70 hover:bg-white/10"
                  }`}
                >
                  CAM {String(i + 1).padStart(2, "0")} · {CAMERA_POSITIONS[i % CAMERA_POSITIONS.length].label}
                </button>
              ))}
            </div>
          </div>
...
function LiveFeed({ bus, cam }) {
  ...
  return (
    <div className="relative aspect-video w-full bg-ink overflow-hidden">
      ...
      <div className="absolute inset-0" style={{ background: "linear-gradient(180deg, #0F172E 0%, #1F2E52 45%, #0F172E 100%)" }} />
      ...
      <div className="absolute bottom-0 left-0 right-0 h-[30%]">
        ...
        <span className="cctv-car cctv-car-2" style={{ animationDelay: "1.2s" }} />
        <span className="cctv-car cctv-car-3" style={{ animationDelay: "2.4s" }} />
      </div>
      {/* overlay grid */}
      ...
      <div className="absolute top-3 left-4 flex items-center gap-2">
        ...
        <span className="text-[11px] font-bold text-alert tracking-widest">REC</span>
      ...
      <div className="absolute bottom-3 left-4 flex items-center gap-2">
        <span className="px-2 py-0.5 rounded bg-black/40 text-white text-[10px] font-mono font-semibold">{bus.id}-CAM{String(cam + 1).padStart(2, "0")}</span>
        <span className="px-2 py-0.5 rounded bg-success/80 text-white text-[10px] font-mono font-semibold">MPDMR</span>
        <span className="px-2 py-0.5 rounded bg-white/10 text-white/70 text-[10px] font-mono">LIVE CCTV</span>
      </div>
